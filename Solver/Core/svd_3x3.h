...
namespace lcs
{
	...
	namespace Local
	{
		...
		static inline void multAB(float a11,
			float						a12,
			float						a13,
			float						a21,
			float						a22,
		...
		static inline void multAtB(float a11,
			float						 a12,
			float						 a13,
			float						 a21,
			float						 a22,
		...
		static inline void quatToMat3(
			const float* qV, float& m11, float& m12, float& m13, float& m21, float& m22, float& m23, float& m31, float& m32, float& m33)
		{
			float w = qV[3];
			float x = qV[0];
			float y = qV[1];
		...
		static inline void jacobiConjugation(
			const int x, const int y, const int z, float& s11, float& s21, float& s22, float& s31, float& s32, float& s33, float* qV)
		{
			float ch, sh;
		...
		static inline float dist2(float x, float y, float z)
		{
			return x * x + y * y + z * z;
		}
